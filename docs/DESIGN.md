...
- Every interest item (Steam game, favorite title, reading entry, character) has a **144 × 120** SVG footprint. README declares both width and height. Interest images form an unboxed gallery, so each image does not need a border and background.
- Character portraits are 64 × 74 within that footprint. Covers and game images use `meet` to preserve the whole artwork. Titles stay at 12 px and remain linked to their full source title. Progress and playtime use a quieter 10 px caption.
